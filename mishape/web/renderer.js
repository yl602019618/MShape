...
export class Viewer {
  ...
  draw(){
    ...
    if(this.cage&&!this.explode){gl.disable(gl.DEPTH_TEST);const pts=this.cagePoints();const lines=[];
      if(Array.isArray(this.cage.edges)){const byId=new Map(pts.map(p=>[p.id,p]));for(const [a,b] of this.cage.edges){const start=byId.get(a),end=byId.get(b);if(start&&end)lines.push(...start.position,...end.position);}}
      else{const grid=this.cage.grid||[4,3,3],byIndex=new Map(pts.map(p=>[p.index.join(','),p]));for(const p of pts)for(let a=0;a<3;a++){const ix=[...p.index];ix[a]++;if(ix[a]>=grid[a])continue;const other=byIndex.get(ix.join(','));if(other)lines.push(...p.position,...other.position);}}
